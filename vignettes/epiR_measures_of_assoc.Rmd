...
Note that the numeric estimate of the odds ratio for a case-control study is exactly the same as that calculated for a cohort study. The expression of the result is the only thing that differs. Key points:
...
An unmeasured confounder would need to be associated with both smoking and low birth weight by a risk ratio of at least 3.7 to fully explain away the observed Mantel-Haenszel incidence risk ratio of 2.1. A confounder of this magnitude would likely already be known from the literature, so if no such variable has been identified, it is unlikely that the observed association is entirely attributable to residual confounding.
