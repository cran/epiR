epi.stratasize <- function (strata.n, strata.mean, strata.var, epsilon, method = "mean", conf.level = 0.95) 
{
    N. <- 1 - ((1 - conf.level) / 2)
    ...
    if (method == "total") {
        ...
        total.sample <- round((((z^2 * N)/(1 + gamma)) * V) / (((z^2 * V) / (1 + gamma)) + N * (epsilon^2)), digits = 0)
    ...
    if (method == "proportion") {
        # Where method == "proportion" the estimated proportions for each strata are entered into the vector strata.mean:
        N <- sum(strata.n)
        mean <- sum(strata.n * strata.mean) / N
        ...
        total.sample <- round(((z^2/N^2) * sigma.bx)/((epsilon^2) + ((z^2/N^2) * sigma.bxd)), digits = 0)
        strata.sample <- round(strata.n * (total.sample/N), digits = 0)
    ...
    if (method == "pps") {
        ...
        total.sample <- round(((z^2/N) * sigma.bx)/(epsilon^2 + ((z^2/N^2) * sigma.bx)), digits = 0)
        strata.sample <- round(strata.n * (total.sample/N), digits = 0)
