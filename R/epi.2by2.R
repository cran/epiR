"epi.2by2" <- function(dat, method = "cohort.count", conf.level = 0.95, units = 100, homogeneity = "breslow.day", outcome = "as.columns"){
    ...
    ## Create measures within the elements list:
    ...
    res <- list(
        ...
        RR.strata = with(elements, data.frame(est = RR.p, se = RR.se, weight = RR.w, lower = RR.l, upper = RR.u)),

        ## Incidence rate ratio:
        IRR.strata = with(elements, data.frame(est = IRR.p, se = IRR.se, weight = IRR.w, lower = IRR.l, upper = IRR.u)),

        ## Odds ratio:
        OR.strata = with(elements, data.frame(est = OR.p, se = OR.se, weight = OR.w, lower = OR.l, upper = OR.u)),

        ## Corrected incidence risk ratio:
        cRR.strata = with(elements, data.frame(est = cRR.p, lower = cRR.l, upper = cRR.u)),

        ## Attributable risk:
        ARisk.strata = with(elements, data.frame(est = ARisk.p, se = ARisk.se, weight = ARisk.w, lower = ARisk.l, upper = ARisk.u)),

        ## Attributable rate:
        ARate.strata = with(elements, data.frame(est = ARate.p, se = ARate.se, lower = ARate.l, upper = ARate.u)),

        ## Attributable fraction for risk data:
        AFRisk.strata = with(elements, data.frame(est = AFRisk.p, lower = AFRisk.l, upper = AFRisk.u)),

        ## Attributable fraction for rate data:
        AFRate.strata = with(elements, data.frame(est = AFRate.p, lower = AFRate.l, upper = AFRate.u)),

        ## Estimated attributable fraction:
        AFest.strata = with(elements, data.frame(est = AFest.p, lower = AFest.l, upper = AFest.u)),

        ## Population attributable risk:
        PARisk.strata = with(elements, data.frame(est = PARisk.p, se = PARisk.se, lower = PARisk.l, upper = PARisk.u)),

        ## Population attributable rate:
        PARate.strata = with(elements, data.frame(est = PARate.p, se = PARate.se, lower = PARate.l, upper = PARate.u)),

        ## Population attributable fraction for risk data:
        PAFRisk.strata = with(elements, data.frame(est = PAFRisk.p, lower = PAFRisk.l, upper = PAFRisk.u)),

        ## Population attributable fraction for rate data:
        PAFRate.strata = with(elements, data.frame(est = PAFRate.p, lower = PAFRate.l, upper = PAFRate.u)),

        ## Estimated population attributable fraction:
        PAFest.strata = with(elements, data.frame(est = PAFest.p, lower = PAFest.l, upper = PAFest.u)),

        ## Crude incidence risk ratio:
        RR.crude = with(elements, data.frame(est = cRR.p, se = cRR.se, lower = cRR.l, upper = cRR.u)),

        ## Crude incidence rate ratio:
        IRR.crude = with(elements, data.frame(est = cIRR.p, se = cIRR.se, lower = cIRR.l, upper = cIRR.u)),

        ## Crude odds ratio:
        OR.crude = with(elements, data.frame(est = cOR.p, se = cOR.se, lower = cOR.l, upper = cOR.u)),

        ## Crude attributable risk:
        ARisk.crude = with(elements, data.frame(est = cARisk.p, se = cARisk.se, lower = cARisk.l, upper = cARisk.u)),

        ## Crude attributable rate:
        ARate.crude = with(elements, data.frame(est = cARate.p, se = cARate.se, lower = cARate.l, upper = cARate.u)),

        ## Crude attributable fraction for risk data:
        AFRisk.crude = with(elements, data.frame(est = cAFRisk.p, lower = cAFRisk.l, upper = cAFRisk.u)),

        ## Crude attributable fraction for rate data:
        AFRate.crude = with(elements, data.frame(est = cAFRate.p, lower = cAFRate.l, upper = cAFRate.u)),

        ## Crude estimated attributable fraction:
        AFest.crude = with(elements, data.frame(est = cAFest.p, lower = cAFest.l, upper = cAFest.u)),

        ## Crude population attributable risk:
        PARisk.crude = with(elements, data.frame(est = cPARisk.p, se = cPARisk.se, lower = cPARisk.l, upper = cPARisk.u)),

        ## Crude population attributable rate:
        PARate.crude = with(elements, data.frame(est = cPARate.p, se = cPARate.se, lower = cPARate.l, upper = cPARate.u)),

        ## Crude population attributable fraction for risk data:
        PAFRisk.crude = with(elements, data.frame(est = cPAFRisk.p, lower = cPAFRisk.l, upper = cPAFRisk.u)),

        ## Crude population attributable fraction for rate data:
        PAFRate.crude = with(elements, data.frame(est = cPAFRate.p, lower = cPAFRate.l, upper = cPAFRate.u)),

        ## Crude estimated population attributable fraction:
        PAFest.crude = with(elements, data.frame(est = cPAFest.p, lower = cPAFest.l, upper = cPAFest.u)),

        ## Mantel-Haenszel adjusted incidence risk ratio:
        RR.mh = with(elements, data.frame(est = sRR.p, se = sRR.se, lower = sRR.l, upper = sRR.u)),

        ## Mantel-Haenszel adjusted incidence rate ratio:
        IRR.mh = with(elements, data.frame(est = sIRR.p, se = sIRR.se, lower = sIRR.l, upper = sIRR.u)),

        ## Mantel-Haenszel adjusted odds ratio:
        OR.mh = with(elements, data.frame(est = sOR.p, se = sOR.se, lower = sOR.l, upper = sOR.u)),

        ## Mantel-Haenszel adjusted attributable risk:
        ARisk.mh = with(elements, data.frame(est = sARisk.p, se = sARisk.se, lower = sARisk.l, upper = sARisk.u)),

        ## Mantel-Haenszel adjusted attributable rate:
        ARate.mh = with(elements, data.frame(est = sARate.p, se = sARate.se, lower = sARate.l, upper = sARate.u)),

        ## Effect of confounding for risk ratio (Woodward p 172):
        RR.conf = with(elements, data.frame(est = RR.conf.p, lower = RR.conf.l, upper = RR.conf.u)),

        ## Effect of confounding for rate ratio (Woodward p 172):
        IRR.conf = with(elements, data.frame(est = IRR.conf.p, lower = IRR.conf.l, upper = IRR.conf.u)),

        ## Effect of confounding for odds ratio (Woodward p 172):
        OR.conf = with(elements, data.frame(est = OR.conf.p, lower = OR.conf.l, upper = OR.conf.u)),

        ## Effect of confounding for attributable risk (Woodward p 172):
        ARisk.conf = with(elements, data.frame(est = ARisk.conf.p, lower = ARisk.conf.l, upper = ARisk.conf.u)),

        ## Effect of confounding for attributable rate (Woodward p 172):
        ARate.conf = with(elements, data.frame(est = ARate.conf.p, lower = ARate.conf.l, upper = ARate.conf.u)),

        ## Chi-squared tests:
        chisq.strata = with(elements, data.frame(test.statistic = chi2, df = 1, p.value = p.chi2)),
        chisq.crude = with(elements, data.frame(test.statistic = chi2s, df = 1, p.value = p.chi2s)),
    ...
    if(length(dim(dat)) > 2){
        res$chisq.mh <- with(elements, data.frame(test.statistic = chi2m, df = 1, p.value = p.chi2m))
    }
    ...
    if(method == "cohort.count" & length(elements$a) == 1){
       ...
       OR    = res$OR.strata,
       AR    = res$ARisk.strata,
       ARp   = res$PARisk.strata,
       AFe   = res$AFRisk.strata,
    ...
    if(method == "cohort.count" & length(elements$a) > 1){
       ...
       ## Verbose part:
       rval <- list(
       RR.strata    = res$RR.strata,
       RR.crude     = res$RR.crude,
       RR.mh        = res$RR.mh,
       OR.strata    = res$OR.strata,
       OR.crude     = res$OR.crude,
       OR.mh        = res$OR.mh,

       AR.strata    = res$ARisk.strata,
       AR.crude     = res$ARisk.crude,
       AR.mh        = res$ARisk.mh,

       ARp.strata   = res$PARisk.strata,
    ...
    if(method == "cohort.time" & length(elements$a) == 1){
       ...
       IRR   = res$IRR.strata,
       AR    = res$ARate.strata,
       ARp   = res$PARate.strata,
       AFe   = res$AFRate.strata,
       AFp   = res$PAFRate.strata,
       chisq = res$chisq.strata)
    ...
    if(method == "case.control" & length(elements$a) == 1){
       ...
       AFest = res$AFest.strata,
       AFp   = res$PAFest.strata,
       chisq = res$chisq.strata)
    ...
    if(method == "cross.sectional" & length(elements$a) > 1){
       ...
       ## Verbose part:
       rval <- list(
       PR.strata    = res$RR.strata,
       PR.crude     = res$RR.crude,
       PR.mh        = res$RR.mh,
...
print.epi.2by2 <- function(x, ...) {
    ...
    if(x$method == "cohort.count" & length(x$elements$a) == 1){
        ...
        with(x$res, {
            ...
            cat(sprintf("\nAttrib fraction in exposed (%%)               %.2f (%.2f, %.2f)",
                        AFRisk.crude$est   * 100,
                        AFRisk.crude$lower * 100,
                        AFRisk.crude$upper * 100
                        ))

            cat(sprintf("\nAttrib fraction in population (%%)            %.2f (%.2f, %.2f)",
                        PAFRisk.strata$est   * 100,
                        PAFRisk.strata$lower * 100,
                        PAFRisk.strata$upper * 100
                        ## PAFRisk.crude$est   * 100,
                        ## PAFRisk.crude$lower * 100,
                        ## PAFRisk.crude$upper * 100
                        ))
        })
        cat("\n---------------------------------------------------------")
        cat("\n", "*", x$res$count.units, "\n")
    ...
    if(x$method == "cohort.time" & length(x$elements$a) == 1){
        ...
        with(x$res, {
            ...
            cat(sprintf("\nAttrib fraction in exposed (%%)               %.2f (%.2f, %.2f)",
                        AFRate.crude$est   * 100,
                        AFRate.crude$lower * 100,
                        AFRate.crude$upper * 100
                        ))

            cat(sprintf("\nAttrib fraction in population (%%)            %.2f (%.2f, %.2f)",
                        PAFRate.crude$est   * 100,
                        PAFRate.crude$lower * 100,
                        PAFRate.crude$upper * 100
                        ))
    ...
    if(x$method == "case.control" & length(x$elements$a) == 1){
        ...
        with(x$res, {
            ...
            cat(sprintf("\nAttrib fraction (est) in exposed  (%%)        %.2f (%.2f, %.2f)",
                        AFest.crude$est   * 100,
                        AFest.crude$lower * 100,
                        AFest.crude$upper * 100
                        ))

            cat(sprintf("\nAttrib fraction (est) in population (%%)      %.2f (%.2f, %.2f)",
                        PAFest.strata$est   * 100,
                        PAFest.strata$lower * 100,
                        PAFest.strata$upper * 100
                        ## PAFest.crude$est   * 100,
                        ## PAFest.crude$lower * 100,
                        ## PAFest.crude$upper * 100
                        ))
        })
        cat("\n---------------------------------------------------------")
        cat("\n", "*", x$res$count.units, "\n")
    ...
    if(x$method == "case.control" & length(x$elements$a) > 1){
        ...
        with(x$res, {
            ...
            cat(sprintf("\nAttrib prevalence (crude:M-H)                %.2f",
                        ## ARate.conf$est       # Same result as the original, but wrong?
                        ARisk.conf$est          # Correct?
                        ))
        })
        cat("\n---------------------------------------------------------")
        cat("\n", "*", x$res$count.units, "\n")
    ...
    if(x$method == "cross.sectional" & length(x$elements$a) == 1){
